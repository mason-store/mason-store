  ...
  <div class="general-dialog">
    ...
    >
      <div>
        ...
        >
          <el-row>
            <el-col :span="item.span" v-for="(item, index) in modelConf.formConfig" :key="index">
              ...
              >
                <div v-for="(imgItem, imgIndex) in winFormData[item.prop]" :key="imgIndex" class="dialog-img">
                  <el-image :preview-src-list="winFormData[item.prop]" :z-index="imgIndex + 1" :src="imgItem" />
                  <span class="cancel" @click="onRemoveImgItem(item.prop, imgIndex)">
                    ×
                  </span>
                </div>
                <el-upload
                  ...
                  :on-success="(response, file, fileList) => handleUploadSuc(item, response, file, fileList)"
                  :before-upload="(response, file, fileList) => handleBeforeUpload(item, response, file, fileList)"
                  v-if="winFormData[item.prop] ? (winFormData[item.prop].length < item.limit ? true : false) : false"
                >
                  <el-button size="mini" plain type="primary" :loading="uploadLoadin">
                    上传图片
                  </el-button>
                </el-upload>
              </el-form-item>
